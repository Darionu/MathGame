...
class Logger {
    constructor() {
        ...
        this._console = true;
        /* 1 - all (info and upper) 2 - info and warn and error, 3 - warn and error, 4 - error only */
        this._acceptableLevel = LogConstants.debug;
    }
    ...
    _dbLogging(logLevel, log, filename) {
        if (Meteor.isServer) {
            dbLogging(
                ...
                log,
                filename,
                LogConstants.serverSide,
                LogConstants.serverSide,
                moment().format(DateFormat.fullDateTimeFormat)
            );
        } else {
            const userId = Meteor.user() ? Meteor.user()._id : LogConstants.unknownUser;
            const userName = Meteor.user() ? Meteor.user().username : LogConstants.unknownUser;
            Meteor.call(
                ServerMethodsNames.log,
                logLevel,
                log,
                filename,
                userId,
